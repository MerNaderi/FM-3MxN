                      ...
                      print = TRUE)
{
  ...
  dmatrix.norm = function(Y, M, S, Ps, Log = F) {
    p = dim(Y)[1]
    n = dim(Y)[2]
    Ps.inv = solve(Ps)
    S.inv = solve(S)
    f.int = function(Y) {
      Delta = tr(Ps.inv %*% t(Y - M) %*% S.inv %*% (Y - M))
      C = (p / 2) * log(det(Ps.inv)) + (n / 2) * log(det(S.inv)) - (n * p / 2) * log(2 * pi)
        #(det(Ps.inv) ^ (p / 2)) * (det(S.inv) ^ (n / 2)) / (2 * pi) ^ (n * p / 2)
      Out = exp(-Delta / 2 + C)
      ifelse(Log == F, return(Out), return(C - Delta / 2))
    }
    if (is.matrix(Y))
      Out = f.int(Y)
    else{
  ...
  dMVN.mix = function(Y, M, S, Ps, PI, Log = T) {
    N = dim(Y)[3]
    Mat =  matrix(0, length(PI), N)
    for (i in 1:length(PI))
      Mat[i, ] = dmatrix.norm(Y, M[, , i], S[, , i], Ps[, , i])
    Out = PI %*% Mat
    Out[which(Out == 0)] <- .Machine$double.xmin
  ...
  if (fix.rho.row) {rho.row = rep(rho.row[1], G)} else {rho.row = rho.row[1:G]}
  if (fix.rho.col) {rho.col = rep(rho.col[1], G)} else {rho.col = rho.col[1:G]}
  if (fix.sigma.row) {sigma.row = rep(sigma.row[1], G)} else {sigma.row = sigma.row[1:G]}
  if (fix.sigma.col) {sigma.col = rep(sigma.col[1], G)} else {sigma.col = sigma.col[1:G]}
  ...
  lk = logli.old = sum(dMVN.mix(Y, M, S, Ps, pp, Log = T))
  ...
  if (isTRUE(print)) {
    ...
    cat(rule(
      center = col_green(
        "* Fitting Mixture of matrix-variate noraml distributions with ",
        row.variance,
  ...
  repeat {
    ...
    ### E step
    Zhat = PLOG = matrix(NA, nrow = N, ncol = G)
    for (i in 1:G) {
      PLOG[, i] = log(pp[i]) + dmatrix.norm(Y, M = M[,, i], S = S[,, i], Ps = Ps[,, i], Log = T)
    }
    ...
    for (i in 1:G) {
      ...
      if (M.const == "I") {
        ...
        SumY = 0
        for (k in 1:N) {
          SumY = SumY + Zhat[k, i] * tr(S.inv %*% Y[, , k] %*% Ps.inv %*% t(All.ones(p, n)))
        }
    ...
    logli.new = sum(dMVN.mix(Y, M, S, Ps, pp, Log = T))
  ...
  #matrix(apply(Z, 1, order), length(pp))[length(pp),]
  di = table(Group)
  ...
  if (isTRUE(print)) {
    ...
    cat(rule(
      center = col_green(
        '** The process of fitting MIX-MVN distributions with the specified ',
