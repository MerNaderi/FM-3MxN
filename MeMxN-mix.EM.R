                        ...
                        print = TRUE)
{
  ...
  f.MMNE = function(Y, M, L, S, Ps, nu, Log = FALSE) {
    p = dim(Y)[1]
    n = dim(Y)[2]
    Ps.inv = solve(Ps)
    S.inv = solve(S)
    eta = sqrt(tr(Ps.inv %*% t(L) %*% S.inv %*% L))
    f.int = function(Y) {
      A = (tr(Ps.inv %*% t(L) %*% S.inv %*% (Y - M)) - nu) / eta
      out = log(nu / eta) + 0.5 * log(2 * pi) + 0.5 * A ^ 2 + pnorm(A, log.p = T) -
        0.5 * tr(Ps.inv %*% t(Y - M) %*% S.inv %*% (Y - M)) -
  ...
  f.MMNE.mix = function(Y, M, L, S, Ps, nu, PI, Log = FALSE) {
    N = dim(Y)[3]
  ...
  if (isTRUE(print)) {
    ...
    cat(rule(
      center = col_green(
        "* Fitting Mixture of matrix-variate MMNE distributions with ",
        row.variance,
  ...
  repeat {
    ...
    for (i in 1:G) {
      Ps.inv = solve(Ps[, , i])
      S.inv = solve(S[, , i])
      eta = sqrt(tr(Ps.inv %*% t(L[, , i]) %*% S.inv %*% L[, , i]))
      A = c()
      for (j in 1:N) {
        A[j] = (tr(Ps.inv %*% t(L[, , i]) %*% S.inv %*% (Y[, , j] - M[, , i])) - nu[i]) /
          eta
      }
      w[, i] = A / eta + HF(A) / eta
      t[, i] = A * w[, i] / eta + 1 / eta ^ 2
    }
  ...
  if (isTRUE(print)) {
    ...
    cat(rule(
      center = col_green(
        '** The process of fitting MIX-MV-MMNE distributions with the specified ',
