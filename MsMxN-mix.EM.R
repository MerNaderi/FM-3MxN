                        ...
                        print = TRUE)
{
  ...
  f.MMNC = function(Y, M, L, S, Ps, nu, Log = FALSE) {
    p = dim(Y)[1]
    n = dim(Y)[2]
    Ps.inv = solve(Ps)
    S.inv = solve(S)
    eta1 = tr(Ps.inv %*% t(L) %*% S.inv %*% L) + (1 + nu) ^ (-2)
    eta2 = tr(Ps.inv %*% t(L) %*% S.inv %*% L) + (1 - nu) ^ (-2)

    F1 = function(x, eta) {
      0.5 * x ^ 2 + pnorm(x, log.p = T) - log(eta)
    }

    f.int = function(Y) {
      A1 = (tr(Ps.inv %*% t(L) %*% S.inv %*% (Y - M))) / sqrt(eta1)
      A2 = (tr(Ps.inv %*% t(L) %*% S.inv %*% (Y - M))) / sqrt(eta2)
      out = log(1 + exp(F1(-A2, sqrt(eta2)) - F1(A1, sqrt(eta1)))) +
        F1(A1, sqrt(eta1)) - 0.5 * tr(Ps.inv %*% t(Y - M) %*% S.inv %*% (Y - M)) -
        (p / 2) * log(det(Ps)) - (n / 2) * log(det(S)) - (p * n / 2) * log(2 * pi)
      #out = ifelse(out == 0, .Machine$double.xmin, out)
      return(out)
    }
    ...
    Out = unlist(lapply(YY, function(x)
      f.int(x)))
    ifelse(Log == FALSE, return(exp(Out)), return(Out))
  }

  f.MMNC.mix = function(Y, M, L, S, Ps, nu, PI, Log = FALSE) {
    N = dim(Y)[3]
    ...
    ifelse(Log == FALSE, return(Out), return(log(Out)))
  }
  ...
  if (isTRUE(print)) {
    ...
    cat(rule(
      center = col_green(
        "* Fitting Mixture of matrix-variate MMSPN distributions with ",
        row.variance,
  ...
  repeat {
    ...
    ### E step
    Zhat = PLOG = w = t = Pii = matrix(NA, nrow = N, ncol = G)
    for (i in 1:G) {
      PLOG[, i] = log(pp[i]) + f.MMNC(
        Y,
        M = M[, , i],
        L = L[, , i],
        S = S[, , i],
    ...
    for (i in 1:G) {
      Ps.inv = solve(Ps[, , i])
      S.inv = solve(S[, , i])
      eta1 = tr(Ps.inv %*% t(L[, , i]) %*% S.inv %*% L[, , i]) + (1 + nu[i]) ^ (-2)
      eta2 = tr(Ps.inv %*% t(L[, , i]) %*% S.inv %*% L[, , i]) + (1 - nu[i]) ^ (-2)

      A1 = A2 = gnum = c()
      for (j in 1:N) {
        A1[j] = (tr(Ps.inv %*% t(L[, , i]) %*% S.inv %*% (Y[, , j] - M[, , i]))) /
          sqrt(eta1)
        A2[j] = (tr(Ps.inv %*% t(L[, , i]) %*% S.inv %*% (Y[, , j] - M[, , i]))) /
          sqrt(eta2)

        gnum[j] = sqrt(eta1 / eta2) * exp(0.5 * (A2[j] ^ 2 - A1[j] ^ 2) +
                                            pnorm(-A2[j], log.p = T) -
                                            pnorm(A1[j], log.p = T))
      }

      SS1 = A1 / sqrt(eta1) + HF(A1) / sqrt(eta1)
      SS2 = A2 / sqrt(eta2) - HF(-A2) / sqrt(eta2)
      ...
      SS1 = A1 / sqrt(eta1) * SS1 + 1 / eta1
      SS2 = (A2 / sqrt(eta2)) ^ 2 + 1 / eta2 - HF(-A2) * A2 / eta2
      t[, i] = g1 * SS1 + g2 * SS2
    }
    ...
    for (i in 1:G) {
      ...
      if (fix.rho.row == FALSE) {
        if (fix.sigma.row == FALSE) {
      ...
      if (fix.rho.col == FALSE) {
        if (fix.sigma.col == FALSE) {
      ...
      Estim = try(nlminb(nu[i], nu.fn, lower = -0.9999, upper = 0.9999))
      if(!('try-error' %in% class(Estim))){
        nu[i] = Estim$par
      } else {nu[i] = nu[i]}
    }
    ...
    if (fix.nu){
      ...
      Estim = try(nlminb(nu, nu.fn, lower = -0.9999, upper = 0.9999))
      if(!('try-error' %in% class(Estim))){
        nu = Estim$par
      } else {nu = nu}
      nu = rep(nu, G)
  ...
  if (isTRUE(print)) {
    ...
    cat(rule(
      center = col_green(
        '** The process of fitting MIX-MV-MMSPN distributions with the specified ',
