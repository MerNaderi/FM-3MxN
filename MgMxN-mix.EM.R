                        ...
                        print = TRUE)
{
  ...
  f.MMNG = function(Y, M, L, S, Ps, nu, Log = FALSE) {
    p = dim(Y)[1]
    n = dim(Y)[2]
    Ps.inv = solve(Ps)
    S.inv = solve(S)
    eta = tr(Ps.inv %*% t(L) %*% S.inv %*% L)
    f.int = function(Y) {
      A = (tr(Ps.inv %*% t(L) %*% S.inv %*% (Y - M)) - nu) / sqrt(eta)
      out = log(nu ^ 2 / eta) + 0.5 * log(2 * pi) + 0.5 * A ^ 2 + pnorm(A, log.p = TRUE) +
  ...
  f.MMNG.mix = function(Y, M, L, S, Ps, nu, PI, Log = FALSE) {
    N = dim(Y)[3]
  ...
  if (isTRUE(print)) {
    ...
    cat(rule(
      center = col_green(
        "* Fitting Mixture of matrix-variate MMNG distributions with ",
        row.variance,
  ...
  repeat {
    ...
    for (i in 1:G) {
      Ps.inv = solve(Ps[, , i])
      S.inv = solve(S[, , i])
      eta = tr(Ps.inv %*% t(L[, , i]) %*% S.inv %*% L[, , i])
      A = c()
      for (j in 1:N) {
        A[j] = (tr(Ps.inv %*% t(L[, , i]) %*% S.inv %*%
                     (Y[, , j] - M[, , i])) - nu[i]) / sqrt(eta)
      }
  ...
  if (isTRUE(print)) {
    ...
    cat(rule(
      center = col_green(
        '** The process of fitting MIX-MV-MMNG distributions with the specified ',
