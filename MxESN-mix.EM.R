                       ...
                       print = TRUE)
{
  ...
  f.RSN = function(Y, M, L, S, Ps, nu, Log = FALSE) {
    p = dim(Y)[1]
    n = dim(Y)[2]
    Ps.inv = solve(Ps)
    S.inv = solve(S)
    eta = sqrt(tr(Ps.inv %*% t(L) %*% S.inv %*% L) + 1)
    f.int = function(Y) {
      A = (tr(Ps.inv %*% t(L) %*% S.inv %*% (Y - M))) / eta
      out = 0.5 * A ^ 2 - log(eta) + pnorm(nu * eta + A, log.p = T) -
        pnorm(nu, log.p = T) - 0.5 * tr(Ps.inv %*% t(Y - M) %*% S.inv %*% (Y - M)) -
  ...
  if (isTRUE(print)) {
    ...
    cat(rule(
      center = col_green(
        "* Fitting Mixture of ", family, " distributions with ",
        row.variance,
  ...
  repeat {
    ...
    for (i in 1:G) {
      Ps.inv = solve(Ps[, , i])
      S.inv = solve(S[, , i])
      eta = sqrt(tr(Ps.inv %*% t(L[, , i]) %*% S.inv %*% L[, , i]) + 1)
      A = c()
      for (j in 1:N) {
      ...
      w[, i] = A / eta + HF(eta * nu[i] + A) / eta
      t[, i] = A ^ 2 / eta ^ 2 + 1 / eta ^ 2 + HF(eta * nu[i] + A) * (A /
                                                                        eta - nu[i]) / eta
    }
    ...
    for (i in 1:G) {
      ...
      if (family != "MVrSN") {
        if (!fix.nu) {
          ...
          Estim = try(nlminb(nu[i], nu.fn, lower = -Inf, upper = Inf))
          if(!('try-error' %in% class(Estim))){
            nu[i] = Estim$par
          } else { nu[i] = nu[i] }
        }
    ...
    if (family != "MVrSN") {
      if (fix.nu) {
        ...
        nu = nu[1]
        Estim = try(nlminb(nu, nu.fn, lower = -Inf, upper = Inf))
        if(!('try-error' %in% class(Estim))){
          nu = Estim$par
        } else { nu = nu }
        nu = rep(nu, G)
  ...
  if (family == "MVrSN")
    m = G * (1 + Mpar + Lpar + Rpar + Cpar) - 1
  if (fix.nu && family != "MVrSN")
    m = m - G + 1
  if (fix.sigma.row)
    m = m - G + 1
  if (fix.sigma.col)
  ...
  if (print) {
    ...
    cat(rule(
      center = col_green(
        '** The process of fitting MIX-', family,' distributions with the specified ',
